...
public class CourseLikeDeliverStrategy implements CouponDeliverStrategy {
    ...
    public boolean canDeliver(User user, Coupon coupon, Map<String, String> extra) {
        ...
        System.out.println("课程 "+courseId+"用户 "+user.getId()+"点赞数 "+result);
        if (result == 1) {
            // 判断优惠券的适用范围
            Course course = courseMapper.selectByPrimaryKey(courseId);
           //List<Course> allCourse=courseMapper.selectAll();
            // 判断能否适用
//            for(Course course:allCourse){
//                System.out.println("用户 "+user.getId()+"优惠课程 "+course.getId()+"优惠券 "+coupon.getId());
                if (coupon.getScope().canUse(course, coupon)) {
                    System.out.println("发放："+"用户 "+user.getId()+"优惠券 "+coupon.getId());
                    return true;
                }
//            }
            return true;
